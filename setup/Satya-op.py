from input satya-op
run git//github.com/SATYA-OP/root/userbot;
await(input("LegendBot"srt(Owner=Its_LegendBoy(os.event:))).get/root/userbot
      Satya=LegendBot(Owner(await:Print(-legendbot).event
                            (try)
                            pass : Legend=Satya(int(Mirror_Mod=False,
                                                    mode=satya,input=userbot.user_id
                      pass 
                            satya
